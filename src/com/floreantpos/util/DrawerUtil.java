...
public class DrawerUtil {
	...
	public static void kickDrawer() {
		...
		if (portName == null || portName.trim().isEmpty()) {
			logger.info("Cash drawer port not configured — skipping drawer kick."); //$NON-NLS-1$
			return;
		}
	...
	public static void kickDrawer(final String portName, final char[] codes) {
		if (portName == null || portName.trim().isEmpty()) {
			return;
		}
		// Run in a daemon thread so that a native-level crash in jSSC does not
		// propagate back to the calling (UI) thread.  The thread is daemonised
		// so it never prevents JVM shutdown.
		Thread t = new Thread(() -> {
			try {
				...
				logger.warn("Cash drawer serial port error on " + portName + ": " + ex.getMessage()); //$NON-NLS-1$ //$NON-NLS-2$
			} catch (Throwable ex) {
				logger.error("Unexpected error kicking cash drawer on " + portName, ex); //$NON-NLS-1$
			}
		}, "DrawerKickThread"); //$NON-NLS-1$
		t.setDaemon(true);
		t.start();
	}
