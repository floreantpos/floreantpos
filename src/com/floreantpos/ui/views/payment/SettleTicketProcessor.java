...
public class SettleTicketProcessor implements CardInputListener {
	...
	private void payUsingCard(String cardName, final double tenderedAmount) throws Exception {
		try {
			...
			CardEntryDialog cardEntryDialog = new CardEntryDialog(this);
			cardEntryDialog.pack();
			cardEntryDialog.open();
		} catch (Exception e) {
	...
	public void cardInputted(CardInputProcessor inputter, PaymentType selectedPaymentType) {
		...
		try {
			...
			if (inputter instanceof CardEntryDialog) {

				CardEntryDialog ced = (CardEntryDialog) inputter;

				switch (ced.getActiveTab()) {
					...
					case KEYED:
						...
						transaction.setCardNumber(ced.getCardNumber());
						transaction.setCardExpMonth(ced.getExpMonth());
						transaction.setCardExpYear(ced.getExpYear());
						transaction.setCardCode(ced.getCvv());
						setTransactionAmounts(transaction);
					...
					case SWIPE:
						String cardStr = ced.getCardString();
						if (StringUtils.isEmpty(cardStr) || cardStr.length() < 16) {
							throw new RuntimeException(Messages.getString("SettleTicketDialog.16"));
						}
						if (!confirmPayment()) return;
						transaction.setCardType(paymentType.getDisplayString());
						transaction.setCardTrack(cardStr);
						transaction.setCaptured(false);
						transaction.setCardMerchantGateway(paymentGateway.getProductName());
					...
					case AUTH_CODE:
						String authCode = ced.getAuthorizationCode();
						if (StringUtils.isEmpty(authCode)) {
							throw new PosException(Messages.getString("SettleTicketDialog.17"));
						}
						PosTransaction authTx = selectedPaymentType.createTransaction();
						authTx.setTicket(ticket);
						authTx.setCardType(selectedPaymentType.getDisplayString());
						authTx.setCaptured(true);
						authTx.setCardReader(CardReader.EXTERNAL_TERMINAL.name());
						authTx.setCardAuthCode(authCode);
						authTx.setCardMerchantGateway(paymentGateway.getProductName());
						setTransactionAmounts(authTx);
						settleTicket(authTx);
						break;
				}

			} else if (inputter instanceof SwipeCardDialog) {
				...
				transaction.setCardCode(mDialog.getCvv());
				setTransactionAmounts(transaction);
	...
	private void showTrainingModePaymentNotice() {
		String message = "<html><b>TRAINING MODE \u2013 NO REAL PAYMENT PROCESSED</b><br><br>" //$NON-NLS-1$
				+ "This is a simulated transaction.<br>" //$NON-NLS-1$
				+ "No card has been charged.</html>"; //$NON-NLS-1$
		JOptionPane.showMessageDialog(
				com.floreantpos.util.POSUtil.getFocusedWindow(),
				message,
				"Training Mode", //$NON-NLS-1$
