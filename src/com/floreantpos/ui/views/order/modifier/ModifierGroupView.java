...
public class ModifierGroupView extends JPanel implements ComponentListener {
	...
	public ModifierGroupView(ModifierSelectionModel modifierSelectionModel) {
		...
		simpleScrollPane.getScrollUpButton().setPreferredSize(new Dimension(PosUIManager.getSize(130), TerminalConfig.getTouchScreenButtonHeight()));
		simpleScrollPane.getScrollDownButton().setPreferredSize(new Dimension(PosUIManager.getSize(130), TerminalConfig.getTouchScreenButtonHeight()));

		add(simpleScrollPane);

		modifierGroupButtonGroup = new ButtonGroup();
		setPreferredSize(PosUIManager.getSize(150, 100));
	...
	protected AbstractButton createItemButton(Object item) {
		...
		button.setPreferredSize(PosUIManager.getSize(130, 80));
	...
	private class ModifierGroupButton extends POSToggleButton implements ActionListener {
		...
		ModifierGroupButton(ModifierGroup menuModifierGroup) {
			setUI(new com.floreantpos.swing.CardPosButtonUI());
			setOpaque(false);
			setContentAreaFilled(false);
			putClientProperty("orderScreenAccent", Boolean.TRUE); //$NON-NLS-1$
			if (com.floreantpos.swing.ButtonStyleConfig.isAccentMode()) {
				setForeground(new java.awt.Color(0x1A, 0x1A, 0x1A));
			}
