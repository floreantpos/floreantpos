...
public class AuthorizeDotNetProcessor implements CardProcessor {
	public void preAuth(PosTransaction transaction) throws Exception {
		...
		CreditCard creditCard = applyCardCode(createCard(transaction), transaction);
	...
	private void populateResult(Result<Transaction> result, PosTransaction transaction) {
		...
		try {
			String track1 = card.getTrack1();
			String track2 = card.getTrack2();
			if (track1 != null && track2 != null) {
				String rawTrackData = "%" + track1 + "?;" + track2 + "?";
				BankCardMagneticTrack track = BankCardMagneticTrack.from(rawTrackData);
				if (track.getTrack1() != null && track.getTrack1().getName() != null) {
...
//	}
	...
	private CreditCard createCard(String cardNumber, String expMonth, String expYear, String cardType) {
		...
		creditCard.setCreditCardNumber(cardNumber.replaceAll("[^0-9]", "")); //$NON-NLS-1$

		return creditCard;
	}

	private CreditCard applyCardCode(CreditCard creditCard, PosTransaction transaction) {
		String cvv = transaction.getCardCode();
